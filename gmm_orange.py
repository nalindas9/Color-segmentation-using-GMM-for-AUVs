"""
Expectation Maximization Algorithm for orange buoy segmentation
...
class GmmEm:
  ...
  def m_step(self):
    ...
    if self.log_likelihood[0]- log_likelihood[0] == 0 or self.log_likelihood[1]-log_likelihood[1] == 0 or self.log_likelihood[2]-log_likelihood[2] == 0:
      ...
      return False
  # Method to train EM algorithm        
  def train(self, iterations):
    for i in range(iterations):
      print('Iteration:', i+1)
      for count, pic in enumerate(glob.glob("/home/nalindas9/Documents/Courses/Spring_2020_Semester_2/ENPM673_Perception_for_Autonomous_Robots/Github/project3/Color-segmentation-using-GMM/DATANEW/Orange_buoy"+ "/*"), 1):
        print('Image:', pic.split("Orange_buoy/",1)[1])
        img = cv2.imread(pic) 
        k = 3
        ...
        if converge == True:
          ...
          axs[1, 0].plot(xaxis, np.array(y1), linewidth=2, color='b')
          axs[1, 0].plot(xaxis, np.array(y2), linewidth=2, color='b')
          axs[1, 0].plot(xaxis, np.array(y3), linewidth=2, color='b')  
          axs[1, 1].plot(xaxis, y1, linewidth=2, color='b')
          axs[1, 1].plot(xaxis, y2, linewidth=2, color='b')
          axs[1, 1].plot(xaxis, y3, linewidth=2, color='b')
  ...
  def test(self):
    ...
    out = cv2.VideoWriter('Orange_buoy.avi',cv2.VideoWriter_fourcc(*'XVID'), 5, (640,480))
    ...
    while cap.isOpened():
      ...
      pixel_probs1 = chance(red,'p1')
      pixel_probs2 = chance(red,'p2')
      ...
      new_prob_image = sum_pixel_probs.reshape(red.shape[0], red.shape[1])
      new_prob_image[new_prob_image>(np.max(new_prob_image)/1.1)] = 255 
      new_prob_image[new_prob_image<=(np.max(new_prob_image)/1.1)] = 0
      ...
      for contour in cnts:
        ...
        if radius > 13:
          center = (int(x),int(y)+2)
          radius = int(radius)
          circles.append([center, radius])
          
      cv2.circle(frame,circles[0][0],circles[0][1],(0,165,255),5)
      out.write(frame)
