...
class GmmEm:
  # Initializing with the 4 gaussians which would like to mix
  def __init__(self, k):
    self.k = k
    ...
    # Variable to store the probalities of the datapoint lying in all four gaussians
    self.probs = []
  ...
  def e_step(self):
    ...
    for pixel in self.channel2:
      ...
      # Expectation of point to lie in Gaussian 4
  ...
  def m_step(self):
    ...
    for clusterr,sumc in enumerate(sum_clusters,0):
      # Calculate fraction of point's belonging to each cluster 1,2,3 and 4
